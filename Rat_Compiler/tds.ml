...
(*supprimer l'info dont le nom associé est name*)
  let supprimer (td:tds) (name:string) =
    match td with 
    | Courante(_,c) -> Hashtbl.remove c (name)
    | Nulle -> failwith "empty! what to remove ?!"
...
let modifier_type_variable t (i:info_ast) =
    match !i with
...
let modifier_type_fonction t tp (i:info_ast) =
       match !i with
...
(* Modifie l'emplacement (dépl, registre) si c'est une InfoVar, ne fait rien sinon *)
 let modifier_adresse_variable d b (i:info_ast) =
     match !i with
