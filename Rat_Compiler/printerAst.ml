...
struct
  ...
  let rec string_of_expression e =
    ...
    | Addresse(n) -> "&"^n^" "
    | Ternaire(ec,e1,e2) -> "(" ^ string_of_expression ec ^ "?" ^ string_of_expression e1 ^
  ...
  let rec string_of_instruction i =
    ...
    | BreakId(n) -> "Break " ^ n ^ " "
    | Continue -> "Continue "
    | ContinueId(n) -> "Continue " ^ n ^ " " 
    | Loop(blc) -> "LOOP "^ "\n" ^(List.fold_right (fun i tq -> (string_of_instruction i)^tq) blc "") ^ "\n"
    | LoopId(n,blc) -> n ^ " : LOOP " ^ "\n" ^ (List.fold_right (fun i tq -> (string_of_instruction i)^tq) blc "")^ "\n" 
    | ConditionnelleSansElse(exp,blc) -> "IF " ^ string_of_expression exp ^ "\n" ^ 
    (List.fold_right (fun i tq -> (string_of_instruction i)^tq) blc "")^ "\n" 
