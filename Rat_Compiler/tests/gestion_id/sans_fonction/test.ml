...
let%test_unit "nv_testPointeur1" = 
  let _ = compiler (pathFichiersRat^"nv_testPointeur1.rat") in ()

let%test_unit "nv_testPointeur2" = 
  try
  let _ = compiler (pathFichiersRat^"nv_testPointeur2.rat") 
    in raise ErreurNonDetectee
  with MauvaiseUtilisationIdentifiant("x") -> ()


let%test_unit "nv_testLoop1" = 
  let _ = compiler (pathFichiersRat^"nv_testLoop1.rat") in ()

let%test_unit "nv_testLoop2" = 
  try
  let _ = compiler (pathFichiersRat^"nv_testLoop2.rat") 
    in raise ErreurNonDetectee
  with ErreurMasquage("l") -> ()

let%test_unit "nv_testLoop3" = 
  let _ = compiler (pathFichiersRat^"nv_testLoop3.rat") in ()

let%test_unit "nv_testLoop4" = 
  let _ = compiler (pathFichiersRat^"nv_testLoop4.rat") in ()

let%test_unit "nv_testLoop5" = 
  try
  let _ = compiler (pathFichiersRat^"nv_testLoop5.rat") 
    in raise ErreurNonDetectee
  with IdentifiantNonDeclare("k") -> ()
