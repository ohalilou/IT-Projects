...
let%test_unit "nv_testtype1"=
  try 
  let _ = compiler (pathFichiersRat^"nv_testtype1.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Int, Rat) -> ()

let%test_unit "nv_testtype2"=
  try 
  let _ = compiler (pathFichiersRat^"nv_testtype2.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Rat, Pointeur(Rat)) -> ()

let%test_unit "nv_testtype3"=
  try 
  let _ = compiler (pathFichiersRat^"nv_testtype3.rat")
    in raise ErreurNonDetectee
  with
